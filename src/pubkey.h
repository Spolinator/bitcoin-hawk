...
private:
    std::array<unsigned char, SIZE> m_pubkey;
...
public:
    ...
    CQuantumPubKey(std::span<const unsigned char> _vch) noexcept;

    // Read-only access for serialization.
    const unsigned char* data() const { return m_pubkey.data(); }
    static constexpr size_t size() { return SIZE; }
    auto begin() const { return m_pubkey.cbegin(); }
    auto end() const { return m_pubkey.cend(); }
    ...
    bool VerifyQuantum(const uint256& hash, std::span<const unsigned char> sigbytes) const;
};
